...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // If no session, email confirmation is required - user stays on page with message
      }
    } catch (err: any) {
  ...
  const handleOAuthSignup = async (provider: 'google' | 'apple' | 'facebook' | 'github') => {
    try {
      const supabase = createClient();
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Or{' '}
            <Link href={`/${locale}/login`} className="font-medium text-primary-600 hover:text-primary-500">
