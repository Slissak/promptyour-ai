...
from shared_python.llm_providers.openrouter_provider import OpenRouterProvider, LLMProviderError
from shared_python.llm_providers.lm_studio_provider import LMStudioProvider, LMStudioError
...
class UnifiedLLMProvider:
    ...
    def __init__(self):
        ...
        self.preferred_provider = getattr(
            settings, "PREFERRED_LLM_PROVIDER", ProviderType.AUTO
        )
    ...
    async def call_model(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            if provider_type == ProviderType.LM_STUDIO:
                # Create a new request without reasoning parameters for LM Studio
                lm_studio_request = LLMRequest(
                    model=request.model,
                    system_prompt=request.system_prompt,
                    user_message=request.user_message,
                    max_tokens=request.max_tokens,
                    temperature=request.temperature,
                    enable_reasoning=False  # Explicitly disable for LM Studio
                )
                return await self._call_lm_studio(lm_studio_request)
            else:
            ...
            "providers": {
                "openrouter": {
                    ...
                    "api_key_configured": bool(
                        ...
                        and self.openrouter.api_key != "your_openrouter_key_here"
                    ),
                },
                "lm_studio": {**lm_studio_status, "type": "local"},
            },
        }
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        if lm_studio_ok or openrouter_ok:
            ...
            status["providers"]["lm_studio"]["status"] == "unavailable"
            and status["providers"]["openrouter"]["api_key_configured"]
        ):
    ...
    async def get_recommended_setup(self) -> Dict[str, Any]:
        ...
        return {"current_setup": status, "recommendations": recommendations}
