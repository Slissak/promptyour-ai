...
from shared_python.types.llm import LLMRequest, LLMResponse
...
class LMStudioProvider:
    ...
    def __init__(self):
        ...
        self.api_key = getattr(
            settings, "LM_STUDIO_API_KEY", "lm-studio" # LM Studio default
        )  # LM Studio default
    ...
    async def call_model(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            async with httpx.AsyncClient(
                timeout=120.0
            ) as client:  # Longer timeout for local inference
            ...
            logger.error(
                "LM Studio connection failed - is LM Studio running?", error=str(e)
            )
            raise LMStudioError(
                f"LM Studio connection failed: {e}. Make sure LM Studio is running on {self.base_url}"
            )
        ...
        try:
            ...
            # Local inference is free, but we can still calculate it
            cost = self.cost_calculator.calculate_cost(request.model, prompt_tokens, completion_tokens)
            ...
            logger.error(
                "Failed to parse LM Studio response", error=str(e), response=data
            )
    ...
    def _extract_thinking(self, content: str) -> Tuple[str, Optional[str]]:
        ...
        thinking_matches = re.findall(
            thinking_pattern, content, re.DOTALL | re.IGNORECASE
        )
        if thinking_matches:
        ...
        reasoning_matches = re.findall(
            reasoning_pattern, content, re.DOTALL | re.IGNORECASE
        )
        if reasoning_matches:
            thinking_content = "\n\n".join(reasoning_matches)
            final_answer = re.sub(
                reasoning_pattern, "", content, flags=re.DOTALL | re.IGNORECASE
            ).strip()
            logger.info(
        ...
        if md_thinking_matches:
            ...
            final_answer = re.sub(
                md_thinking_pattern, "", content, flags=re.DOTALL
            ).strip()
            logger.info(
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if is_healthy:
                    ...
                    return {
                        "status": "healthy",
                        "url": self.base_url,
                        "loaded_model": loaded_model.get("id")
                        if loaded_model
                        else None,
                        "model_display_name": loaded_model.get("display_name")
                        if loaded_model
                        else None,
                    }
                else:
