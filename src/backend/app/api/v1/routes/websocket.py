...
from backend.app.api.v1.dependencies.auth import get_current_user_id_from_query_optional
from backend.app.api.v1.dependencies.services import get_websocket_chat_handler
...
async def websocket_chat_endpoint(
    websocket: WebSocket,
    user_id: str = Depends(get_current_user_id_from_query_optional),
    conversation_id: Optional[str] = Query(
        None, description="Optional conversation ID"
    ),
    chat_handler: WebSocketChatHandler = Depends(get_websocket_chat_handler),
):
    """
    ...
    await chat_handler.handle_connection(
        websocket=websocket, user_id=user_id, conversation_id=conversation_id
    )
...
async def websocket_admin_endpoint(
    ...
    try:
        await connection_manager.connect(
            websocket=websocket, connection_id=connection_id, user_id="admin"
        )
        ...
        while True:
            try:
                ...
                if data == "get_stats":
                    ...
                    await connection_manager.send_personal_message(
                        {"type": "broadcast_result", "sent_to": sent_count},
                        connection_id,
                    )

                else:
                    await connection_manager.send_personal_message(
                        {"type": "error", "message": f"Unknown admin command: {data}"},
                        connection_id,
                    )
