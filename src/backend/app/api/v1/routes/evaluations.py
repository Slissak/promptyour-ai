...
from backend.app.services.evaluation_scheduler import evaluation_scheduler
from backend.app.services.model_evaluation_scanner import ModelEvaluationScanner
from backend.app.models.schemas import ThemeType, UserRating
...
async def rate_message(rating: UserRating) -> Dict[str, Any]:
    """Rate a message response"""
    logger.info(
        f"Received rating for message {rating.message_id}: {rating.rating}/5",
        extra={
...
async def trigger_evaluation_scan(
    ...
    scan_type: str = Query(
        default="full_scan",
        description="Type of scan: full_scan, incremental, leaderboard_only",
    ),
) -> Dict[str, Any]:
...
async def get_model_rankings(
    ...
    limit: int = Query(
        default=20, le=100, description="Maximum number of models to return"
    ),
    sort_by: str = Query(
        default="overall", description="Sort by: overall, theme_score, cost_efficiency"
    ),
) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "rankings": {"overall": top_models[:limit], "by_theme": theme_leaders},
        }
...
async def get_theme_recommendations(theme: str) -> Dict[str, Any]:
    ...
    try:
        ...
        recommendations = evaluation_scheduler.model_selector.get_theme_recommendations(
            theme_enum
        )
        ...
        recommendations["data_freshness"] = evaluation_summary.get(
    ...
    try:
        if not evaluation_scheduler.model_selector:
            ...
            if evaluation_scheduler.is_running:
                ...
                await evaluation_scheduler.start_scheduler(
                    evaluation_scheduler.model_selector
                )

        return {"message": "Evaluation scheduler started", "status": "running"}
...
async def stop_scheduler() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"message": "Evaluation scheduler stopped", "status": "stopped"}
