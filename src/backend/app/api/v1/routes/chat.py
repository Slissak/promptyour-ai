...
from backend.app.services.chat_service import ChatService, ChatServiceError
from backend.app.api.v1.dependencies.services import get_chat_service
from backend.app.core.logging import get_logger
from shared_python.config.config_loader import get_config_loader
from backend.app.api.v1.dependencies.auth import get_current_user_id_optional
...
async def send_message(
    ...
    logger.info(
        ...
        request_body=user_input.model_dump(),
        request_headers=request.headers,
    )

    try:
        response = await chat_service.process_user_request(user_input, user_id)
...
async def send_quick_message(
    ...
    logger.info(
        ...
        request_body=quick_input.model_dump(),
        request_headers=request.headers,
    )

    try:
        response = await chat_service.process_quick_request(quick_input, user_id)
        ...
        logger.error(
            "Unexpected error in quick chat endpoint", user_id=user_id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error",
...
async def send_raw_message(
    ...
    try:
        ...
        logger.error(
            "Unexpected error in RAW chat endpoint", user_id=user_id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Internal server error",
...
user_id: str = Depends(get_current_user_id_optional)
    ...
    if rating.message_id != message_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Message ID mismatch"
        )

    logger.info(
        "Rating received", message_id=message_id, user_id=user_id, rating=rating.rating
    )
...
async def chat_health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={"service": "unhealthy", "error": str(e)},
        )
