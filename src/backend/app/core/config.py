...
class Settings(BaseSettings):
    ...
    LM_STUDIO_URL: str = Field(
        default="http://localhost:1234/v1", description="LM Studio API URL"
    )
    LM_STUDIO_API_KEY: str = Field(
        default="lm-studio", description="LM Studio API key (usually not needed)"
    )
    PREFERRED_LLM_PROVIDER: str = Field(
        default="auto",
        description="Preferred LLM provider (openrouter, lm_studio, auto)",
    )
    PREFER_LOCAL_LLM: bool = Field(
        default=False, description="Prefer local LLM over cloud when available"
    )
    ...
    ALLOWED_HOSTS: List[str] = Field(
        default=["localhost", "127.0.0.1", "test"], description="Allowed hosts"
    )
    ...
    # Cost Management
    DEFAULT_DAILY_BUDGET: float = Field(
        default=10.0, description="Default daily budget"
    )
    DEFAULT_MONTHLY_BUDGET: float = Field(
        default=100.0, description="Default monthly budget"
    )

    # Model Configuration
    DEFAULT_MODEL: str = Field(
        default="claude-3-haiku", description="Default LLM model"
    )
    FALLBACK_MODEL: str = Field(
        default="gpt-3.5-turbo", description="Fallback LLM model"
    )
