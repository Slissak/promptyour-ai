...
class ConfigManager:
    ...
    def _get_default_config_path(self) -> str:
        ...
        possible_paths = [
            ...
            os.path.join(
                os.path.dirname(__file__), "../../../config/evaluation_config.yaml"
            ),
        ]
    ...
    async def update_theme_weights(self, theme: str, weights: Dict[str, float]) -> bool:
        ...
        try:
            ...
            if abs(total_weight - 1.0) > 0.1:
                logger.warning(
                    f"Theme weights for {theme} sum to {total_weight}, not 1.0"
                )
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "model_config": {
                "cost_tiers": {"budget": 0.01, "balanced": 0.05, "premium": 1.0}
            },
            "scoring": {
                "score_combination": {"static_weight": 0.4, "dynamic_weight": 0.6}
            },
            "features": {
    ...
    def _cleanup_backups(self, keep_count: int = 5):
        ...
        try:
            ...
            backup_files = glob.glob(
                os.path.join(
                    backup_dir, f"{os.path.basename(self.config_file)}.backup.*"
                )
            )
