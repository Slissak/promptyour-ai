...
def _create_enum_from_config(
    enum_name: str, config_loader_method: str, fallback_values: List[str] = None
) -> type:
...
# Add docstrings
ThemeType.__doc__ = (
    "Themes loaded from config/themes.yaml - Edit that file to modify options"
)
AudienceType.__doc__ = (
    "Audiences loaded from config/audiences.yaml - Edit that file to modify options"
)
...
class ChatMessage(BaseModel):
    ...
    provider: Optional[str] = Field(
        None, description="Provider used for assistant messages"
    )
...
class QuickInput(BaseModel):
    ...
    question: str = Field(
        ..., description="The user's main question or request", min_length=1
    )
    ...
    force_provider: Optional[str] = Field(
        None, description="Force use of specific provider"
    )
...
class UserInput(BaseModel):
    ...
    theme: Optional[ThemeType] = Field(
        None, description="Selected theme from dropdown (optional)"
    )
    audience: Optional[AudienceType] = Field(
        None,
        description="Target audience for response (optional, defaults to adults if not specified)",
    )
    response_style: Optional[ResponseStyle] = Field(
        None,
        description="Response style preference for output formatting and length (optional, defaults to structured_detailed if not specified)",
    )
    context: Optional[str] = Field(
        None, description="Additional context sentences provided by user"
    )
    conversation_id: Optional[str] = Field(
        None, description="Conversation ID for history"
    )
...
class ProcessedContext(BaseModel):
    ...
    processing_confidence: float = Field(
        ..., description="Confidence in auto-detection 0-1"
    )
    requires_clarification: bool = Field(
        default=False, description="Whether input needs user clarification"
    )
...
class LLMRequest(BaseModel):
    ...
    reasoning_budget_tokens: Optional[
        int
    ] = None  # For Anthropic models: token budget (min 1024)
...
class ChatResponse(BaseModel):
    ...
    raw_response: Optional[str] = Field(
        None, description="RAW model response (no system prompt) for comparison"
    )
    raw_tokens_used: Optional[int] = Field(None, description="Tokens used by RAW response")
    raw_cost: Optional[float] = Field(None, description="Cost of RAW response")
    thinking: Optional[str] = Field(
        None,
        description="Internal model reasoning/thinking (stored but not displayed to user)",
    )
...
class RawInput(BaseModel):
    ...
    question: str = Field(
        ...,
        description="The user's question - sent directly to model without any prompt engineering",
        min_length=1,
    )
    conversation_id: Optional[str] = Field(
...
class RawResponse(BaseModel):
    ...
    system_prompt: str = Field(
        default="", description="System prompt (always empty for RAW responses)"
    )
    thinking: Optional[str] = Field(
        None,
        description="Internal model reasoning/thinking (stored but not displayed to user)",
