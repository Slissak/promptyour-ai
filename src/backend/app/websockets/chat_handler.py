...
from backend.app.models.schemas import UserInput, ThemeType, AudienceType, UserRating
...
class WebSocketChatHandler:
    ...
    async def handle_connection(
        self, websocket: WebSocket, user_id: str, conversation_id: Optional[str] = None
    ):
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logger.info(
                        "WebSocket disconnected normally", connection_id=connection_id
                    )
                    break
                    ...
                    await self._send_error(
                        connection_id, f"Message handling error: {e}"
                    )
    ...
    async def _handle_chat_request(
        self, connection_id: str, user_id: str, message: dict
    ):
        ...
        try:
            ...
            if "message_history" in data and data["message_history"]:
                from backend.app.models.schemas import ChatMessage, MessageRole
            ...
            await self._send_step_update(
                connection_id, request_id, "input_processing", "Analyzing your input..."
            )
            ...
            logger.error(
                "Chat request failed", connection_id=connection_id, error=str(e)
            )
    ...
    async def _send_step_update(
        self, connection_id: str, request_id: str, step: str, message: str
    ):
    ...
    async def _send_debug_comparison(
        self, connection_id: str, request_id: str, comparison_data: dict
    ):
        ...
        logger.info(
            ...
            enhanced_tokens=comparison_data.get("enhanced_response", {}).get(
                "tokens_used", 0
            ),
            basic_tokens=comparison_data.get("basic_response", {}).get(
                "tokens_used", 0
            ),
        )

    async def _handle_user_rating(
        self, connection_id: str, user_id: str, message: dict
    ):
        ...
        try:
            ...
            logger.error(
                "Rating submission failed", connection_id=connection_id, error=str(e)
            )
    ...
    async def _handle_cancel_request(
        ...
        if connection_id in self.active_requests:
            ...
            logger.info(
                "Request cancelled", connection_id=connection_id, request_id=request_id
            )
        else:
    ...
    async def _handle_get_history(
        ...
        try:
            ...
            await self._send_error(
                connection_id, f"Failed to get conversation history: {e}"
            )
