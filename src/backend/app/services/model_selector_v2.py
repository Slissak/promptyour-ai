...
class ThemeBasedModelSelector:
    ...
    async def select_model(
        self, context: ProcessedContext, budget_tier: str = "balanced"
    ) -> ModelChoice:
        ...
        complexity_filtered = self._filter_by_complexity(
            theme_candidates, context.inferred_complexity
        )
    ...
    def _get_theme_candidates(self, theme: ThemeType, budget_tier: str) -> List[str]:
        ...
        if budget_tier == "premium":
            ...
            return (
                budget_models
                + [m for m in primary_models if m not in budget_models][:4]
            )
        else:  # balanced
    ...
    def _filter_by_complexity(
        self, candidates: List[str], complexity: str
    ) -> List[str]:
    ...
    def _apply_dynamic_scoring(
        self, candidates: List[str], context: ProcessedContext
    ) -> Dict[str, float]:
    ...
    def _calculate_base_score(self, model: str, context: ProcessedContext) -> float:
        ...
        complexity_alignment = self._get_complexity_alignment_score(
            model, context.inferred_complexity
        )
        ...
        subject_alignment = self._get_subject_alignment_score(
            model, context.inferred_subject
        )
    ...
    def _get_subject_alignment_score(self, model: str, subject: str) -> float:
        ...
        if required_strength in strengths:
            return 0.9
        elif "reasoning" in strengths and required_strength in [
            "reasoning",
            "analysis",
        ]:
            return 0.8
    ...
    def _get_dynamic_score(
        self, model: str, context: ProcessedContext
    ) -> Optional[float]:
    ...
    def _select_optimal_model(
        ...
        if budget_tier == "budget":
            ...
            budget_models = [
                ...
                if self.base_models.get(model, {}).get("cost_per_1k_tokens", 1.0)
                <= budget_limit
            ]
            if budget_models:
    ...
    def _create_model_choice(
        ...
        estimated_cost = (context.estimated_tokens / 1000) * model_config[
            "cost_per_1k_tokens"
        ]

        # Generate reasoning
        reasoning = self._generate_selection_reasoning(
            selected_model, context, score, model_config
        )
    ...
    def _generate_selection_reasoning(
        self, model: str, context: ProcessedContext, score: float, config: Dict
    ) -> str:
        ...
        if context.inferred_complexity in self.complexity_models:
            ...
            if model in complexity_config["preferred"]:
                reasons.append(
                    f"well-suited for {context.inferred_complexity} level tasks"
                )
    ...
    def get_model_info(self, model: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_updated": self.last_evaluation_update.isoformat()
            if self.last_evaluation_update
            else None,
        }
    ...
    async def _load_configuration(self):
        ...
        self.theme_model_rankings = {
            ThemeType.ACADEMIC_HELP: {
                "primary": tier_preferences.get(
                    "balanced", ["claude-3-sonnet", "gpt-4"]
                ),
                "budget": tier_preferences.get(
        ...
        self.complexity_models = {
            "beginner": {
                "preferred": tier_preferences.get(
                    "budget", ["claude-3-haiku", "gpt-3.5-turbo"]
                ),
