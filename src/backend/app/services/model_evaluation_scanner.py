...
from backend.app.models.schemas import ThemeType
...
class ModelEvaluationScanner:
    ...
    async def run_full_scan(self) -> Dict[str, Any]:
        ...
        model_evaluations = await self._create_model_evaluations(
    ...
    async def _scrape_source(
        ...
        try:
            if source_config["type"] == "leaderboard":
                return await self._scrape_leaderboard(
                    source_config["url"], scraping_config
                )
                ...
                return await self._scrape_benchmark(
                    source_config["url"], scraping_config
                )
            else:
    ...
    async def _scrape_leaderboard(self, url: str, config: Dict) -> Dict[str, Any]:
        ...
        score_col_indices = {
            col: headers.index(col) for col in config["score_columns"] if col in headers
        }
    ...
    async def _scrape_github_results(self, url: str, config: Dict) -> Dict[str, Any]:
        ...
        models_data = {}
        base_url = url.replace("/blob/main", "/raw/main").replace(
            "/tree/main", "/raw/main"
        )
    ...
    def _normalize_model_name(self, raw_name: str) -> str:
        ...
        name = raw_name.strip()
        name = re.sub(
            r"^(meta-llama/|microsoft/|google/|anthropic/)",
            "",
            name,
            flags=re.IGNORECASE,
        )
        ...
        name_mappings = model_config.get(
            "model_aliases",
            {
                "gpt-4-turbo": "gpt-4",
    ...
    async def _calculate_theme_rankings(
        self, all_model_data: Dict[str, Dict]
    ) -> Dict[str, List[Dict]]:
        ...
        for theme in ThemeType:
            ...
            for source_name, source_data in all_model_data.items():
                ...
                for model_name, model_data in source_data.items():
                    ...
                    overall_score = (
                        model_scores.get("overall") or model_scores.get("Average") or 0
                    )

                    if overall_score > 0:
                        theme_scores[model_name]["weighted_score"] += (
                            overall_score * source_weight
                        )
            ...
            for model_name, score_data in theme_scores.items():
                if score_data["total_weight"] > 0:
                    normalized_score = (
                        score_data["weighted_score"] / score_data["total_weight"]
                    )
                    normalized_scores.append(
                        {
                            ...
                            "sources_count": len(
                                [s for s in theme_weights.keys() if s in all_model_data]
                            ),
                        }
                    )
    ...
    async def _create_model_evaluations(
        self, all_model_data: Dict, theme_rankings: Dict
    ) -> Dict[str, Any]:
        ...
        for model_name in all_models:
            ...
            for source_name, source_data in all_model_data.items():
                if model_name in source_data:
                    ...
                    # Overall score
                    overall_score = (
                        source_scores.get("overall")
                        or source_scores.get("Average")
                        or 0
                    )
                    if overall_score > 0:
            ...
            for theme_name, theme_data in theme_rankings.items():
                theme_model = next(
                    (m for m in theme_data if m["model"] == model_name), None
                )
                if theme_model:
    ...
    async def _load_configuration(self):
        ...
        self.evaluation_sources = config_manager.get_evaluation_sources(
            active_only=True
        )
