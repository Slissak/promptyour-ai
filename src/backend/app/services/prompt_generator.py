...
from backend.app.models.schemas import ProcessedContext, ThemeType, AudienceType, ResponseStyle
...
class ModelSpecificPromptGenerator:
    ...
    async def create_model_specific_prompt(
        ...
        logger.info(
            "Intelligent targeted prompt generated",
            model=model,
    ...
    def _analyze_question_type(self, question: str) -> Dict[str, str]:
        ...
        if any(
            ...
            word in question_lower
            for word in ["how to", "how do", "how can", "steps", "process", "method"]
        ):
            return {"type": "how_to", "approach": "provide step-by-step guidance"}
        elif any(
            word in question_lower for word in ["why", "reason", "because", "cause"]
        ):
            return {
                "type": "reasoning",
                "approach": "explain underlying causes and logic",
            }
        elif any(
            word in question_lower
            ...
            return {
                "type": "comparison",
                "approach": "analyze differences and similarities",
            }
        elif any(
            word in question_lower
            for word in ["solve", "fix", "debug", "problem", "issue", "error"]
        ):
            return {
                "type": "problem_solving",
                "approach": "diagnose and provide solutions",
            }
        elif any(
            word in question_lower
            for word in ["example", "examples", "show me", "demonstrate"]
        ):
            return {
                "type": "examples",
                "approach": "provide concrete examples and demonstrations",
            }
        elif any(
            word in question_lower
            for word in ["best", "recommend", "suggest", "should", "advice"]
        ):
            return {
                "type": "recommendation",
                "approach": "provide expert recommendations with rationale",
            }
        elif any(
            word in question_lower
            for word in ["create", "make", "build", "write", "design"]
        ):
            return {
                "type": "creation",
                "approach": "guide through creation process with examples",
            }
        else:
            return {
                "type": "general",
                "approach": "provide comprehensive, helpful response",
            }

    def _create_expert_persona(
        self, theme: ThemeType, audience: AudienceType
    ) -> Dict[str, str]:
        ...
        persona_desc = theme_experts.get(theme, {}).get(
            audience.value,
    ...
    def _get_formatting_instructions(
        self,
        question_analysis: Dict[str, str],
        ...
        if response_style == ResponseStyle.PARAGRAPH_BRIEF:
            formatting_instructions.append(
                "Format your response as ONE concise paragraph that covers all key points"
            )
            formatting_instructions.append(
                "Avoid bullet points, lists, or multiple sections - keep it flowing and brief"
            )
            formatting_instructions.append(
            ...
            formatting_instructions.append(
                "Provide comprehensive details with examples and thorough explanations"
            )
            formatting_instructions.append(
                "Use numbered lists, bullet points, and clear visual organization"
            )
        elif response_style == ResponseStyle.INSTRUCTIONS_ONLY:
            formatting_instructions.append(
                "Provide ONLY direct instructions or answers without background explanation"
            )
            formatting_instructions.append(
                "Skip context, theory, or 'why' explanations - focus purely on 'what' and 'how'"
            )
            formatting_instructions.append(
                "Be concise and action-oriented without elaboration"
            )
        elif response_style == ResponseStyle.COMPREHENSIVE:
            formatting_instructions.append(
                "Provide comprehensive coverage with background context, detailed explanations, and reasoning"
            )
            formatting_instructions.append(
                "Include examples, alternatives, edge cases, and thorough analysis"
            )
            formatting_instructions.append(
                "Explain the 'why' behind information and provide complete understanding"
            )

        # Question-type specific formatting (secondary layer)
        if (
            question_analysis["type"] == "how_to"
            and response_style != ResponseStyle.PARAGRAPH_BRIEF
        ):
            formatting_instructions.append(
            ...
            if response_style == ResponseStyle.COMPREHENSIVE:
                formatting_instructions.append(
                    "Include detailed explanations for each step and potential pitfalls"
                )
        elif question_analysis["type"] == "comparison" and response_style in [
            ResponseStyle.STRUCTURED_DETAILED,
            ResponseStyle.COMPREHENSIVE,
        ]:
            formatting_instructions.append(
                "Use comparison tables or side-by-side analysis when helpful"
            )
            formatting_instructions.append(
                "Clearly highlight key differences and similarities"
            )
        elif (
            question_analysis["type"] == "problem_solving"
            and response_style != ResponseStyle.INSTRUCTIONS_ONLY
        ):
            formatting_instructions.append(
                "Structure response as: Problem → Analysis → Solutions → Implementation"
            )

        # Audience-specific formatting preferences (tertiary layer)
        if (
            audience in [AudienceType.SMALL_KIDS, AudienceType.TEENAGERS]
            and response_style != ResponseStyle.INSTRUCTIONS_ONLY
        ):
            ...
            audience == AudienceType.UNIVERSITY_LEVEL
            and response_style == ResponseStyle.COMPREHENSIVE
        ):
            formatting_instructions.append(
                "Use academic formatting with clear citations and references"
            )
            formatting_instructions.append(
                "Include detailed explanations with supporting evidence"
            )
        elif (
    ...
    def _get_reasoning_pattern(
        self, question_analysis: Dict[str, str], theme: ThemeType
    ) -> str:
        ...
        if question_analysis["type"] == "problem_solving":
            reasoning_instructions.append(
                "Think through this step-by-step: identify the problem, analyze root causes, generate solutions, evaluate options"
            )
        elif question_analysis["type"] == "reasoning":
            reasoning_instructions.append(
                "Use logical reasoning: present the premise, explain the logic, draw conclusions, consider alternative perspectives"
            )
        elif question_analysis["type"] == "comparison":
            reasoning_instructions.append(
                "Analyze systematically: identify criteria for comparison, evaluate each option against criteria, synthesize findings"
            )
        elif question_analysis["type"] == "explanation":
            reasoning_instructions.append(
                "Build understanding progressively: start with fundamentals, add complexity gradually, connect to existing knowledge"
            )

        # Theme-specific reasoning enhancements
        if theme == ThemeType.ACADEMIC_HELP:
            reasoning_instructions.append(
                "Encourage critical thinking and help the learner discover insights rather than just providing answers"
            )
        elif theme == ThemeType.CODING_PROGRAMMING:
            reasoning_instructions.append(
                "Think algorithmically: break down the problem, consider edge cases, optimize for readability and performance"
            )
        elif theme == ThemeType.RESEARCH_ANALYSIS:
            reasoning_instructions.append(
                "Apply research methodology: consider sources, evaluate evidence quality, acknowledge limitations"
            )
    ...
    def _get_quality_safeguards(self, theme: ThemeType, audience: AudienceType) -> str:
        ...
        safeguard_instructions.append(
            "Verify accuracy: double-check facts and ensure information is current and correct"
        )
        safeguard_instructions.append(
            "Acknowledge uncertainty: be clear about what you're confident about vs. what might need further verification"
        )
        safeguard_instructions.append(
            "Consider bias: reflect on potential biases in your response and present balanced perspectives"
        )

        # Theme-specific safeguards
        if theme in [ThemeType.ACADEMIC_HELP, ThemeType.TUTORING_EDUCATION]:
            safeguard_instructions.append(
                "Focus on learning: guide understanding rather than just providing answers"
            )
        elif theme == ThemeType.CODING_PROGRAMMING:
            safeguard_instructions.append(
                "Emphasize best practices: include security considerations, code maintainability, and testing approaches"
            )
        elif theme == ThemeType.BUSINESS_PROFESSIONAL:
            safeguard_instructions.append(
                "Consider stakeholder impact: address ethical implications and multiple perspectives"
            )

        # Audience-specific safeguards
        if audience == AudienceType.SMALL_KIDS:
            safeguard_instructions.append(
                "Ensure age-appropriateness: avoid complex or potentially concerning content"
            )
        elif audience == AudienceType.PROFESSIONALS:
            safeguard_instructions.append(
                "Emphasize professional responsibility: include relevant compliance and ethical considerations"
            )

        return " ".join(safeguard_instructions)

    def _get_meta_cognitive_instructions(
        self, question_analysis: Dict[str, str]
    ) -> str:
        ...
        if question_analysis["type"] == "how_to":
            meta_instructions.append(
                "Verify practicality: ensure your steps are actionable and realistic"
            )
        elif question_analysis["type"] == "explanation":
            meta_instructions.append(
                "Check understanding: consider if a novice would understand your explanation"
            )
        elif question_analysis["type"] == "problem_solving":
            meta_instructions.append(
                "Validate solutions: ensure your recommendations are feasible and effective"
            )

        return " ".join(meta_instructions)

    def _detect_professional_domain(
        self, question: str, theme: ThemeType
    ) -> Optional[str]:
        ...
        if (
            any(keyword in question_lower for keyword in scientific_keywords)
            or theme == ThemeType.RESEARCH_ANALYSIS
        ):
            return "scientific"
        ...
        if any(
            keyword in question_lower for keyword in educational_keywords
        ) or theme in [ThemeType.ACADEMIC_HELP, ThemeType.TUTORING_EDUCATION]:
            return "educational"
        ...
        if (
            any(keyword in question_lower for keyword in business_keywords)
            or theme == ThemeType.BUSINESS_PROFESSIONAL
        ):
            return "business"
        ...
        if (
            any(keyword in question_lower for keyword in creative_keywords)
            or theme == ThemeType.CREATIVE_WRITING
        ):
            return "creative"

        return None

    def _generate_response_instructions(
        self,
        question: str,
        theme: ThemeType,
        audience: AudienceType,
        ...
        if question_analysis["type"] == "explanation":
            instructions.append(
                f"Define key terms clearly for {audience.value}, then build understanding step by step"
            )
        elif question_analysis["type"] == "how_to":
            instructions.append(
                f"Provide clear, actionable steps that {audience.value} can easily follow"
            )
        elif question_analysis["type"] == "problem_solving":
            instructions.append(
                f"Help diagnose the issue and provide practical solutions suitable for {audience.value}"
            )
        elif question_analysis["type"] == "comparison":
            instructions.append(
                f"Present clear comparisons with examples relevant to {audience.value}"
            )

        # Audience-specific adaptations
        if audience == AudienceType.SMALL_KIDS:
            instructions.append(
                "Use very simple language, fun analogies, and encouraging tone"
            )
        elif audience == AudienceType.TEENAGERS:
            instructions.append(
                "Be engaging and relatable, avoid being preachy, use relevant examples"
            )
        elif audience == AudienceType.UNIVERSITY_LEVEL:
            instructions.append(
                "Include academic rigor, encourage critical thinking, cite sources when relevant"
            )
        elif audience == AudienceType.PROFESSIONALS:
            instructions.append(
                "Focus on practical implementation, efficiency, and real-world application"
            )

        # Theme-specific considerations
        if theme == ThemeType.CODING_PROGRAMMING:
            instructions.append(
                "Include clean, well-commented code examples and explain best practices"
            )
        elif theme == ThemeType.ACADEMIC_HELP:
            instructions.append(
                "Encourage learning and understanding rather than just providing answers"
            )
        elif theme == ThemeType.CREATIVE_WRITING:
            instructions.append(
                "Inspire creativity while providing constructive, specific feedback"
            )
    ...
    def _build_intelligent_prompt(
        ...
        model: str,
        context: ProcessedContext,
        expert_persona: Dict[str, str],
        audience_psychology: Dict[str, str],
        ...
        model_config = self.model_prompts.get(
            model, self.model_prompts["claude-3-sonnet"]
        )

        # Detect professional domain for specialized instructions
        professional_domain = self._detect_professional_domain(
            context.question, context.theme
        )

        # Get advanced instruction components
        formatting_instructions = self._get_formatting_instructions(
            question_analysis, context.audience, context.response_style
        )
        reasoning_pattern = self._get_reasoning_pattern(
            question_analysis, context.theme
        )
        quality_safeguards = self._get_quality_safeguards(
            context.theme, context.audience
        )
        meta_cognitive_instructions = self._get_meta_cognitive_instructions(
            question_analysis
        )
        ...
        prompt_parts.extend(
            [
                ...
                "2. Consider the most effective way to explain this topic for their level",
                "3. Choose examples and analogies they can relate to",
                "4. Structure your response for maximum clarity and engagement",
