...
class TestRedesignedInput:
    ...
    async def test_input_processing(self):
        ...
        for test_case in self.test_cases:
            ...
            if (
                expected_complexity
                and expected_complexity == result.inferred_complexity
            ):
                print(f"   ✅ Complexity matches expectation ({expected_complexity})")
            elif expected_complexity:
                print(
                    f"   ⚠️  Complexity different from expected ({expected_complexity})"
                )
    ...
    async def test_edge_cases(self):
        ...
        edge_cases = [
            {
                ...
                "input": UserInput(
                    question="Help", theme=ThemeType.GENERAL_QUESTIONS, context=None
                ),
            },
            {
        ...
        for test_case in edge_cases:
            ...
            print(
                f"   ✓ Complexity: {result.inferred_complexity} (score: {result.complexity_score:.2f})"
            )
            print(f"   ✓ Confidence: {result.processing_confidence:.2f}")
            print(f"   ✓ Needs Clarification: {result.requires_clarification}")

            if result.requires_clarification:
                print("   ⚠️  System flagged this for clarification (low confidence)")
