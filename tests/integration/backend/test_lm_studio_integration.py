...
class LMStudioIntegrationTester:
                        ...
                        if "status" in endpoint:
                            if "data" in data:
                                ...
                                if "providers" in status_data:
                                    for provider, info in status_data[
                                        "providers"
                                    ].items():
                                        print(
                                            f"   📡 {provider}: {info.get('status', 'unknown')}"
                                        )
                    else:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.get(
                    f"{self.api_base_url}/api/v1/providers/health"
                )
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"   Active providers: {', '.join(health_data['active_providers'])}"
                    )
                    print(
                        f"   Preferred: {health_data.get('preferred_provider', 'auto')}"
                    )
                    print(
                        f"   Prefer local: {health_data.get('prefer_local_models', True)}"
                    )
                else:
    ...
    async def test_model_listing(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.api_base_url}/api/v1/providers/models"
                )
    ...
    async def test_setup_guide(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.api_base_url}/api/v1/providers/setup-guide"
                )
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"✅ Setup guide generated ({len(recommendations)} recommendations)"
                    )
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{self.api_base_url}/api/v1/providers/test"
                )
                ...
                if response.status_code == 200:
                    ...
                    if test_data.get("success"):
                        ...
                        print(
                            f"   Response: {data.get('response_content', '')[:100]}..."
                        )
    ...
    async def test_troubleshooting(self):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.api_base_url}/api/v1/providers/troubleshoot"
                )
    ...
    async def test_chat_with_local_llm(self):
        ...
        try:
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(
                    f"{self.api_base_url}/api/v1/chat/process", json=chat_request
                )
