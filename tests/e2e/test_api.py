...
class MockTest:
    ...
    async def test_model_selection(self):
        ...
        for test_case in self.test_cases:
            ...
            if model_choice.model == expected:
                ...
                print(
                    f"  ⚠ Model different from expected ({expected}), but may be valid"
                )
    ...
    async def test_prompt_generation(self):
        ...
        system_prompt = await generator.create_model_specific_prompt(
            model=model_choice.model, context=context
        )
        ...
        theme_found = (
            theme_text in system_prompt.lower()
            or test_case["input"]["theme"] in system_prompt.lower()
            or context.inferred_subject.lower() in system_prompt.lower()
        )
        ...
        assert (
            theme_found
        ), f"Theme '{theme_text}' or '{test_case['input']['theme']}' or related content not found in prompt"
