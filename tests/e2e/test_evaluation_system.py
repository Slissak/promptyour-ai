...
from backend.app.services.model_evaluation_scanner import ModelEvaluationScanner
from backend.app.services.model_selector_v2 import ThemeBasedModelSelector
from backend.app.services.evaluation_scheduler import EvaluationScheduler
from backend.app.services.input_processor_v2 import UserInputProcessor
from backend.app.models.schemas import UserInput, ThemeType
...
class TestEvaluationSystem:
    ...
    async def test_web_scraping_mock(self):
        ...
        for source_name, config in source_info["sources"].items():
            print(
                f"   • {source_name}: {config.get('type', 'unknown')} ({config.get('url', 'N/A')})"
            )
    ...
    async def test_model_evaluation_processing(self):
        ...
        for theme_name, rankings in list(theme_rankings.items())[:3]:
            ...
            for i, model_data in enumerate(rankings[:3]):
                print(
                    f"      {i+1}. {model_data['model']} (score: {model_data['score']:.1f})"
                )

        # Test model evaluation creation
        model_evaluations = await self.scanner._create_model_evaluations(
            all_model_data, theme_rankings
        )

        print(f"\n📈 Model Evaluations Created: {len(model_evaluations)} models")
        for model_name, eval_data in list(model_evaluations.items())[:2]:
            print(
                f"   • {model_name}: overall={eval_data['overall_score']:.1f}, sources={eval_data['sources_count']}"
            )
    ...
    async def test_dynamic_model_selection(self):
        ...
        for i, test_case in enumerate(test_cases):
            ...
            for budget_tier in ["budget", "balanced", "premium"]:
                model_choice = await self.model_selector.select_model(
                    processed_context, budget_tier
                )
    ...
    async def test_scheduler_functionality(self):
        ...
        if "total_models" in summary:
            ...
            if "top_models_overall" in summary:
                print(
                    f"   • Top 3 Models: {[model[0] for model in summary['top_models_overall'][:3]]}"
                )
    ...
    async def test_theme_optimization(self):
        ...
        for theme in ThemeType:
            ...
            print(
                f"   Primary Models: {recommendations['recommendations']['primary'][:3]}"
            )
            print(
                f"   Budget Models: {recommendations['recommendations']['budget'][:3]}"
            )
            ...
            if theme in sample_questions:
                ...
                print(
                    f"   🤖 Selected: {model_choice.model} (confidence: {model_choice.confidence:.2f})"
                )
    ...
    async def test_integration_flow(self):
        ...
        print(
            f"   Processing Confidence: {processed_context.processing_confidence:.2f}"
        )
        ...
        if model_info["dynamic_scores"]:
            ...
            print("   Using static configuration only")
    ...
    async def test_error_handling(self):
        ...
        processed_context = await self.input_processor.process_input(
            UserInput(
                question="Test question",
                theme=ThemeType.GENERAL_QUESTIONS,
                context="Test",
        ...
        short_input = UserInput(
            question="Hi", theme=ThemeType.GENERAL_QUESTIONS, context=None
        )

        processed_context = await self.input_processor.process_input(short_input)
        print(
            f"✅ Handled short input, complexity: {processed_context.complexity_score:.2f}"
        )
        ...
        print(
            f"✅ Handled long input, estimated tokens: {processed_context.estimated_tokens}"
        )
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            print(
                "   • Scrapes 8+ evaluation sources (HuggingFace, ChatBot Arena, etc.)"
            )
            ...
            print("\n🔧 Next Steps:")
            print(
                "   1. Install web scraping dependencies: pip install beautifulsoup4 aiohttp"
            )
